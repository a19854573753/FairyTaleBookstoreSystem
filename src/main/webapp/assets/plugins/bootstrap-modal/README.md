...
See live demo [here](jschr.github.com/bootstrap-modal/).
...
+ **attentionAnimation**
Set the animation used by the `attention` method. Any animation in [animate.css](daneden.me/animate/) is supported but only the *shake* animation is included by default.
...
If you want to prevent the background page from scrolling (see [demo](jschr.github.com/bootstrap-modal/) for example) you must wrap the page contents in a `<div class="page-container">`. For example:
...
See the stackable example on the [demo](jschr.github.com/bootstrap-modal/) page for an example.
